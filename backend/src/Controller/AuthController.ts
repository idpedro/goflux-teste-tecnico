...
class AuthControler {
  async authenticate(req: Request, resp: Response) {
    ...
    if (!email || !password) {
      return resp
        .status(401)
        .json({ errors: [{ message: "Usuário ou Senha Invalido" }] });
    }
    ...
    if (!isCorrectPass)
      return resp
        .status(401)
        .json({ errors: [{ message: "Usuário ou Senha Inválido" }] });
  ...
  async check(req: Request, resp: Response) {
    ...
    if (!authorization)
      return resp
        .status(401)
        .json({ errors: [{ message: "Sem autorização" }] });
    const [baere, token] = authorization?.split(" ");
    try {
      const payload = jwt.checkToken(token);
      resp.status(200).json({ errors: [{ message: "Autenticado" }] });
    } catch (error) {
      return resp
        .status(401)
        .json({ errors: [{ message: "Sem autorização" }] });
